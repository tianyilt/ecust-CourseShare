...
def _save_png(img, path):
    """cv2.imwrite with Unicode-safe path."""
...
def watershed_segment_single_region(mask,
    ...
    return markers
def process_single_image(img_path, out_dir, debug=False, call_matlab=False):
    ...
    target_dir = os.path.join(out_dir, base_name)
    ...
    cloud_overlay = _colorize_labels(cloud_remap, cloud_n, alpha=0.45, base_img=img, hue_shift=0.0)

    print(f"⛰️  检测到 {mountain_n} 层不同山峰/远近山体")
    mountain_overlay = _colorize_labels(mountain_remap, mountain_n, alpha=0.45, base_img=img, hue_shift=0.5)

    combined = img.copy()
    combined = _colorize_labels(cloud_remap, cloud_n, alpha=0.45, base_img=combined, hue_shift=0.0)
    combined = _colorize_labels(mountain_remap, mountain_n, alpha=0.45, base_img=combined, hue_shift=0.5)

    boundary_mask = (cloud_markers == -1) | (mountain_markers == -1)
    combined[boundary_mask] = [0, 0, 255]

    # 保存过程图片
    _save_png(img, os.path.join(target_dir, 'original.png'))
    _save_png(cloud_mask, os.path.join(target_dir, 'cloud_mask.png'))
    _save_png(mountain_mask, os.path.join(target_dir, 'mountain_mask.png'))
    _save_png(cloud_remap, os.path.join(target_dir, 'cloud_labels.png'))
    _save_png(mountain_remap, os.path.join(target_dir, 'mountain_labels.png'))
    _save_png(cloud_overlay, os.path.join(target_dir, 'cloud_overlay.png'))
    _save_png(mountain_overlay, os.path.join(target_dir, 'mountain_overlay.png'))
    _save_png(combined, os.path.join(target_dir, 'combined_overlay.png'))
    ...
    if call_matlab:
        try:
            ...
            matlab_cmd = f"try, cd('{script_dir_matlab}'); show_results; catch e, disp(getReport(e)); end"
            if debug:
                print('调用 MATLAB 命令:', matlab_cmd)
            try:
                subprocess.Popen(['matlab', '-nosplash', '-r', matlab_cmd])
            except Exception:
                subprocess.run(['matlab', '-nosplash', '-r', matlab_cmd], check=False)
        except FileNotFoundError:
            print('MATLAB 不在 PATH，跳过 MATLAB 展示。')
        except Exception as e:
            print('调用 MATLAB 出错:', str(e))

    print(f"✅ 云+山峰分层分割结果已保存：{target_dir}\n")


def process_folder(folder_path, out_dir, debug=False, call_matlab=False):
    supported_formats = (".jpg", ".jpeg", ".png", ".bmp", ".tiff")
    for file in os.listdir(folder_path):
        if file.lower().endswith(supported_formats):
            img_path = os.path.join(folder_path, file)
            process_single_image(img_path, out_dir, debug=debug, call_matlab=call_matlab)
...
def main():
    ...
    parser.add_argument('--out', '-o', default='output', help='Output directory (default: ./output)')
    parser.add_argument('--debug', action='store_true')
    parser.add_argument('--matlab', action='store_true', help='Call MATLAB show_results.m for visualization')
    ...
    if os.path.isfile(target):
        process_single_image(target, out_dir, debug=args.debug, call_matlab=args.matlab)
    elif os.path.isdir(target):
        process_folder(target, out_dir, debug=args.debug, call_matlab=args.matlab)
    else:
