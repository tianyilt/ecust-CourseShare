...
--（1）请用SQL的
--Grant和Revoke语句（加上视图机制）完成以下授权定义或存取控制功能，并进行相应的安全性测试；
--（a）用户王明对两个表有select权力；
...
--（c）每个职工只对自己的记录有select权力（本道题也可以一个一个来授权）；
...
CREATE VIEW 部门工资 AS
SELECT Section.名称,MAX(工资) as 最高工资,MIN(工资) as 最低工资,AVG(工资) as 平均工资
FROM Worker,Section
WHERE Worker.部门号=Section.部门号
GROUP BY Section.名称
